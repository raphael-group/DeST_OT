...
1. `src/destot/DESTOT/align`: Given a pair of ST slices from two developmental timepoints, infer a spatiotemporal alignment matrix Pi and a growth vector xi. As mentioned in the paper, there are two settings we recommend for this function: the default for growth-rates ($\alpha = 0.2$, $\beta = 0.5$, $\gamma = 50$, $\epsilon = 0.1$), and the robust setting for alignments which are robust to capture-frame cut-off effects ($\alpha = 0.99$, $\beta = 0.6$, $\gamma = 1$, $\epsilon = 0.1$).
